...
class Subject extends REST_Controller{
    ...
    public function getDataByTeach_get(){
        ...
        $result = $this->Subject_model->getDataByTeach($criteria, $db1);
        if(empty($result)){
            $db2 = $this->load->database('nfe2', TRUE);
            $result = $this->Subject_model->getDataByTeach($criteria, $db2);
            if(empty($result)){
                $db3 = $this->load->database('nfe3', TRUE);
                $result = $this->Subject_model->getDataByTeach($criteria, $db3);
            }
        }
        $data['data'] = $result;
    ...
    public function getSemestry_get(){
        ...
        $db1 = $this->load->database('nfe1', TRUE);
        $result = $this->Subject_model->getSemestry($TeachFirstName, $TeachLastName, $db1);
        if(empty($result)){
            $db2 = $this->load->database('nfe2', TRUE);
            $result = $this->Subject_model->getSemestry($TeachFirstName, $TeachLastName, $db2);
            if(empty($result)){
                $db3 = $this->load->database('nfe3', TRUE);
                $result = $this->Subject_model->getSemestry($TeachFirstName, $TeachLastName, $db3);
            }
        }
